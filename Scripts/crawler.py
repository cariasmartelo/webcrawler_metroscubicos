...
class Market:
    ...
    def get_sub_markets(self):
        ...
        if total_properties <= 40 * 50:
            #Max pages displayed * properties per page
            return ([], total_properties)

        submarkets = []
        if self.level == 0:
            submarkets_str = market_soup.find('dl', class_=\
                "filters__group filters__city")
        elif self.level == 1:
            submarkets_str = market_soup.find('dl', class_=\
                "filters__group filters__price")
        elif self.level == 2:
            submarkets_str = market_soup.find('dl', class_=\
                "filters__group filters__COVERED_AREA")
...
def get_total_results(market_soup):
    ...
    Output:
        int
    '''
    results_str = market_soup.find('div', class_="quantity-results")
    if results_str:
...
def get_properties_links(market_soup):
    ...
    Output:
        List of str
    '''
    properties_links = market_soup.find_all('a', class_="item__info-link")
